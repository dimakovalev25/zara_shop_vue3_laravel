<header
        ...
        class="flex justify-between bg-white shadow-md text-black fixed-header transition-all duration-300 ease-in-out "
>
    ...
    categories: {{ json_encode(\App\Http\Helpers\Categories::getCategories()) }}
      ...
      <svg
        ...
        {{--            менюшка мобильная--}}
        <div class="font-bold text-xl block fixed z-10 top-0 bottom-0 height h-full w-[170px] transition-all bg-gray-300 md:hidden pl-6 pr-4 pt-10"
             :class="mobileMenuOpen ? 'left-0' : '-left-[170px]'">
            @if(!Auth::guest())
                <ul>
                    <div x-data="{ searchVisible: false }">
